(defparameter *ccg-grammar*
'(((KEY 1) (PHON I) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |1S|))))) (SEM "I")
   (PARAM 1.0))
  ((KEY 2) (PHON ARRIVES) (MORPH EN)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT NP) (FEATS ((AGR |3S|))))))
   (SEM (LAM X ("ARRIVE" X))) (PARAM 1.0))
  ((KEY 3) (PHON ARRIVED) (MORPH EN)
   (SYN (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X ("ARRIVE" X))) (PARAM 1.0))
  ((KEY 4) (PHON THINK) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |1S|)))))
     (DIR FS) (MODAL HARMONIC) ((BCAT S) (FEATS NIL))))
   (SEM (LAM P (LAM X (("THINK" P) X)))) (PARAM 1.0))
  ((KEY 5) (PHON JOHN) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "JOHN") (PARAM 1.0))
  ((KEY 6) (PHON LIKES) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("LIKE" X) Y)))) (PARAM 1.0))
  ((KEY 7) (PHON AND) (MORPH CO)
   (SYN
    ((((BCAT @X) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT @X) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT @X) (FEATS NIL))))
   (SEM (LAM P (LAM Q (LAM X (("AND" (P X)) (Q X)))))) (PARAM 1.0))
  ((KEY 8) (PHON YOU) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |2S|)))))
   (SEM "YOU") (PARAM 1.0))
  ((KEY 9) (PHON BELIEVE) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |2S|)))))
     (DIR FS) (MODAL HARMONIC) ((BCAT S) (FEATS NIL))))
   (SEM (LAM P (LAM X (("BELIEVE" P) X)))) (PARAM 1.0))
  ((KEY 10) (PHON MARY) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "MARY") (PARAM 1.0))
  ((KEY 11) (PHON HATES) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("HATE" X) Y)))) (PARAM 1.0))
  ((KEY 12) (PHON CATS) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3P|)))))
   (SEM "CATS") (PARAM 1.0))
  ((KEY 13) (PHON THE) (MORPH D)
   (SYN
    (((BCAT NP) (FEATS ((HEAD ?H)))) (DIR FS) (MODAL HARMONIC)
     ((BCAT N) (FEATS ((HEAD ?H))))))
   (SEM (LAM X ("DEF" X))) (PARAM 1.0))
  ((KEY 14) (PHON BOOK) (MORPH N) (SYN ((BCAT N) (FEATS NIL))) (SEM "BOOK")
   (PARAM 1.0))
  ((KEY 15) (PHON "this book") (MORPH N)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
      ((BCAT NP) (FEATS ((TOP P)))))))
   (SEM (LAM P (("AND" (P "BOOK")) ("TOPIC" "BOOK")))) (PARAM 1.0))
  ((KEY 16) (PHON LIKE) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |1S|)))))
     (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("LIKE" X) Y)))) (PARAM 1.0))
  ((KEY 17) (PHON PICKED) (MORPH EN)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT (UP)) (BCONST T) (FEATS NIL)))
     (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((HEAVY M))))))
   (SEM
    (LAM Y
     (LAM X (LAM Z (("CAUSE" ("INIT" (((("HOLD" _) ("REACH" X)) Y) Z))) Z)))))
   (PARAM 1.0))
  ((KEY 18) (PHON PICKED) (MORPH EN)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((LEXC P)))))
     (DIR FS) (MODAL ALL) ((BCAT (UP)) (BCONST T) (FEATS NIL))))
   (SEM (LAM X (LAM Y (LAM Z (((("PICK" _) X) Y) Z))))) (PARAM 1.0))
  ((KEY 19) (PHON THINKS) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL HARMONIC) ((BCAT SP) (FEATS NIL))))
   (SEM (LAM P (LAM X (("THINK" P) X)))) (PARAM 1.0))
  ((KEY 20) (PHON SHE) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "SHE") (PARAM 1.0))
  ((KEY 21) (PHON HARRY) (MORPH NP) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "HARRY") (PARAM 1.0))
  ((KEY 22) (PHON BARRY) (MORPH NP) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "BARRY") (PARAM 1.0))
  ((KEY 23) (PHON MARY) (MORPH NP) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "MARY") (PARAM 1.0))
  ((KEY 24) (PHON UP) (MORPH P)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR BS) (MODAL ALL)
      (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM P ("UP" (P X))))) (PARAM 1.0))
  ((KEY 25) (PHON BEANS) (MORPH N) (SYN ((BCAT N) (FEATS ((HEAD BEANS)))))
   (SEM "BEANS") (PARAM 1.0))
  ((KEY 26) (PHON THAT) (MORPH REL)
   (SYN
    ((((BCAT N) (FEATS ((HEAD ?H)))) (DIR BS) (MODAL ALL)
      ((BCAT N) (FEATS ((HEAD ?H)))))
     (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS ((HEAD ?H)))) (DIR FS) (MODAL ALL)
      ((BCAT NP) (FEATS ((HEAD ?H)))))))
   (SEM (LAM P (LAM Q (LAM X (("AND" (P X)) (Q X)))))) (PARAM 1.0))
  ((KEY 27) (PHON SPILLED) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS ((HEAD BEANS)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((HEAD BEANS))))))
   (SEM (LAM X (LAM Y (((("DIVULGE" _) ("PUBLIC" X)) "SECRET") Y))))
   (PARAM 1.0))
  ((KEY 28) (PHON CAUSED) (MORPH EN)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("CAUSE" X) Y)))) (PARAM 1.0))
  ((KEY 29) (PHON "quite a stir") (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM "STIR") (PARAM 1.0))
  ((KEY 30) (PHON TAROO-WA) (MORPH N) (SYN ((BCAT NP) (FEATS ((TOP P)))))
   (SEM "TAROO") (PARAM 1.0))
  ((KEY 31) (PHON HANAKO-NI) (MORPH N)
   (SYN
    (((BCAT PREDP) (FEATS ((CASE DAT)))) (DIR FS) (MODAL ALL)
     ((((BCAT CING) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (LAM X ((P X) "HANAKO")))) (PARAM 1.0))
  ((KEY 32) (PHON HANAKO-NI) (MORPH N) (SYN ((BCAT NP) (FEATS ((CASE DAT)))))
   (SEM "HANAKO") (PARAM 1.0))
  ((KEY 33) (PHON HUKU-O) (MORPH N)
   (SYN
    (((BCAT PREDP) (FEATS ((CASE ACC)))) (DIR BS) (MODAL ALL)
     ((((BCAT CING) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (P "CLOTHES"))) (PARAM 1.0))
  ((KEY 34) (PHON HUKU-O) (MORPH N) (SYN ((BCAT NP) (FEATS ((CASE ACC)))))
   (SEM "CLOTHES") (PARAM 1.0))
  ((KEY 35) (PHON KITEIRU) (MORPH JP)
   (SYN
    ((((BCAT CING) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("WEARING" X) Y)))) (PARAM 1.0))
  ((KEY 36) (PHON KITEIRU) (MORPH JP)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("WEAR" X) Y)))) (PARAM 1.0))
  ((KEY 37) (PHON YOGOS-ARE-TA) (MORPH JP)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE TOP)))))
      (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE DAT)))))
     (DIR BS) (MODAL ALL) ((BCAT PREDP) (FEATS ((CASE ACC))))))
   (SEM (LAM X (LAM Y (LAM Z ((("LET" (("DIRTY" (X Z)) Y)) Y) Z)))))
   (PARAM 1.0))
  ((KEY 38) (PHON YOGOS-ARE-TA) (MORPH JP)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE TOP)))))
      (DIR BS) (MODAL ALL) ((BCAT PREDP) (FEATS ((CASE DAT)))))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ACC))))))
   (SEM (LAM X (LAM Y (LAM Z ((("LET" (("DIRTY" (X Y)) Y)) Y) Z)))))
   (PARAM 1.0))
  ((KEY 39) (PHON ZHANGSAN) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "Z")
   (PARAM 1.0))
  ((KEY 40) (PHON XIHUAN) (MORPH CH)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("LIKE" X) Y)))) (PARAM 1.0))
  ((KEY 41) (PHON XIHUAN) (MORPH CH)
   (SYN (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (("LIKE" X) "TOP"))) (PARAM 1.0))
  ((KEY 42) (PHON DANSHI) (MORPH CO)
   (SYN
    ((((BCAT @X) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT @X) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT @X) (FEATS NIL))))
   (SEM (LAM P (LAM Q (LAM X (("BUT" (P X)) (Q X)))))) (PARAM 1.0))
  ((KEY 43) (PHON E2R) (MORPH CO)
   (SYN
    ((((BCAT @X) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT @X) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT @X) (FEATS NIL))))
   (SEM (LAM P (LAM Q (LAM X (("AND" (P X)) (Q X)))))) (PARAM 1.0))
  ((KEY 44) (PHON LISI) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "LISI")
   (PARAM 1.0))
  ((KEY 45) (PHON BU) (MORPH NEG)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (LAM X ("NEG" (P X))))) (PARAM 1.0))
  ((KEY 46) (PHON "zhe-ben shu") (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM ("DEF" "BOOK")) (PARAM 1.0))
  ((KEY 47) (PHON TA) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "HESHE")
   (PARAM 1.0))
  ((KEY 48) (PHON DIGEI) (MORPH CH)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (LAM Z ((("GIVE" X) Y) Z))))) (PARAM 1.0))
  ((KEY 49) (PHON GEGE) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "BROTHER")
   (PARAM 1.0))
  ((KEY 50) (PHON "yi-hu jiu") (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM ("ONE" "WINE")) (PARAM 1.0))
  ((KEY 51) (PHON JIEJIE) (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM "SISTER") (PARAM 1.0))
  ((KEY 52) (PHON "yi-pan cai") (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM ("ONE" "DISH")) (PARAM 1.0))
  ((KEY 53) (PHON ZHANGSAN) (MORPH N)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
      ((BCAT NP) (FEATS ((TOP P) (PRO P) (PERS |3S|)))))))
   (SEM (LAM P (("AND" (P Z)) ("TOPIC" Z)))) (PARAM 1.0))
  ((KEY 54) (PHON WO) (MORPH N) (SYN ((BCAT NP) (FEATS ((PERS |1S|)))))
   (SEM "I") (PARAM 1.0))
  ((KEY 55) (PHON QIDAI) (MORPH CH)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM P (LAM Y (("FORESEE" (P X)) Y))))) (PARAM 1.0))
  ((KEY 56) (PHON QUAN) (MORPH CH)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM P (LAM Y ((("PERSUADE" (P X)) X) Y))))) (PARAM 1.0))
  ((KEY 57) (PHON DAYING) (MORPH CH)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM P (LAM Y ((("PROMISE" (P Y)) X) Y))))) (PARAM 1.0))
  ((KEY 58) (PHON TA) (MORPH PRO)
   (SYN
    ((((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
       (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS ((PRO P) (PERS |3S|)))))
     (DIR BS) (MODAL ALL)
     (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
       (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P ("RES" P))) (PARAM 1.0))
  ((KEY 59) (PHON "yao lai") (MORPH CH) (SYN ((BCAT VP) (FEATS NIL)))
   (SEM (LAM X ("WILLCOME" X))) (PARAM 1.0))
  ((KEY 60) (PHON BEN-IM) (MORPH N)
   (SYN
    (((BCAT S) (FEATS ((AGR |1S|)))) (DIR FS) (MODAL ALL)
     ((BCAT IV) (FEATS ((AGR |1S|))))))
   (SEM (LAM P (P "I"))) (PARAM 1.0))
  ((KEY 61) (PHON ADAM-IN) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "MAN") (PARAM 1.0))
  ((KEY 62) (PHON OKU) (MORPH TR)
   (SYN
    (((BCAT IV) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT NP) (FEATS ((CASE ACC))))))
   (SEM (LAM X (LAM Y (("READ" X) Y)))) (PARAM 1.0))
  ((KEY 63) (PHON OKUDU) (MORPH TR)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((CASE NOM) (AGR |3S|)))))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ACC))))))
   (SEM (LAM X (LAM Y (("READ" X) Y)))) (PARAM 1.0))
  ((KEY 64) (PHON -DUGU) (MORPH COMP)
   (SYN
    ((((BCAT S) (FEATS ((AGR ?A)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR ?A)))))
     (DIR BS) (MODAL HARMONIC) ((BCAT IV) (FEATS ((AGR ?A))))))
   (SEM (LAM P (LAM X (P X)))) (PARAM 1.0))
  ((KEY 65) (PHON -NU) (MORPH CASE)
   (SYN
    (((BCAT S) (FEATS ((CASE ACC)))) (DIR BS) (MODAL HARMONIC)
     ((BCAT S) (FEATS ((AGR |3S|))))))
   (SEM (LAM P P)) (PARAM 1.0))
  ((KEY 66) (PHON BIL) (MORPH TR)
   (SYN
    (((BCAT IV) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT S) (FEATS ((CASE ACC))))))
   (SEM (LAM P (LAM X (("KNOW" P) X)))) (PARAM 1.0))
  ((KEY 67) (PHON -DIGIM) (MORPH REL)
   (SYN
    ((((BCAT NP) (FEATS NIL)) (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL HARMONIC)
     (((BCAT S) (FEATS ((AGR |1S|)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (LAM Q (LAM X (("AND" (P X)) (Q X)))))) (PARAM 1.0))
  ((KEY 68) (PHON KITAP) (MORPH N)
   (SYN
    (((BCAT NP) (FEATS NIL)) (DIR BS) (MODAL ALL)
     (((BCAT NP) (FEATS NIL)) (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (P "BOOK"))) (PARAM 1.0))
  ((KEY 69) (PHON "O Paulo") (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "PAUL") (PARAM 1.0))
  ((KEY 70) (PHON NAO) (MORPH NEG)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR ?A)))))
     (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS ((FOP P)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR ?A)))))))
   (SEM (LAM P (LAM X ("NEG" (P X))))) (PARAM 1.0))
  ((KEY 71) (PHON OS) (MORPH PRO)
   (SYN
    ((((BCAT S) (FEATS ((FOP P)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR ?A)))))
     (DIR FS) (MODAL ALL)
     ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((AGR ?A)))))
      (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS ((TYP PRO) (KIND PLU)))))))
   (SEM (LAM P (P "THEM"))) (PARAM 1.0))
  ((KEY 72) (PHON VIU) (MORPH PT)
   (SYN
    ((((BCAT S) (FEATS ((TENSE PAST)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("SEE" X) Y)))) (PARAM 1.0))
  ((KEY 73) (PHON HANKWUK-EY) (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM "KOREA") (PARAM 1.0))
  ((KEY 74) (PHON SEWUL-I) (MORPH N)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
      (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE ?C))))))
     (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
      (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE ?C))))))))
   (SEM (LAM P ("SEUL" P))) (PARAM 1.0))
  ((KEY 75) (PHON NAMTAYMUN-SICANG-I) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE NOM))))) (SEM "MARKET") (PARAM 1.0))
  ((KEY 76) (PHON PUL-I) (MORPH N) (SYN ((BCAT NP) (FEATS ((CASE NOM)))))
   (SEM "FIRE") (PARAM 1.0))
  ((KEY 77) (PHON NA-ASS-TA) (MORPH KO)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE ?C1)))))
      (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ?C2)))))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE NOM))))))
   (SEM
    (LAM X (LAM Y (LAM Z (("AND" ((("BREAKOUT" Z) X) "ONE")) (("AT" Y) Z))))))
   (PARAM 1.0))
  ((KEY 78) (PHON "bayi yara") (MORPH N) (SYN ((BCAT NP) (FEATS ((CASE ABS)))))
   (SEM "MAN") (PARAM 1.0))
  ((KEY 79) (PHON NUMA-NGU) (MORPH N) (SYN ((BCAT NP) (FEATS ((ACSE ERG)))))
   (SEM "FATHER") (PARAM 1.0))
  ((KEY 80) (PHON GIGA-N) (MORPH DY)
   (SYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((CASE ABS)))))
      (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ERG)))))
     (DIR FS) (MODAL ALL) ((BCAT VP) (FEATS NIL))))
   (SEM (LAM P (LAM Y (LAM X ((("TELL" (P X)) X) Y))))) (PARAM 1.0))
  ((KEY 81) (PHON GUBI-NGU) (MORPH N) (SYN ((BCAT NP) (FEATS ((CASE ERG)))))
   (SEM "GUBI") (PARAM 1.0))
  ((KEY 82) (PHON MAWA-LI) (MORPH DY)
   (SYN
    (((BCAT VP) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT NP) (FEATS ((CASE ERG))))))
   (SEM (LAM X (LAM Y (("EXAMINE" Y) X)))) (PARAM 1.0))
  ((KEY 83) (PHON BANAGA-NYU) (MORPH DY) (SYN ((BCAT VP) (FEATS NIL)))
   (SEM (LAM X ("RETURN" X))) (PARAM 1.0))
  ((KEY 84) (PHON BAGUL) (MORPH CL)
   (SYN
    (((BCAT NP) (FEATS ((CASE DAT)))) (DIR FS) (MODAL ALL)
     ((BCAT N) (FEATS ((CASE DAT))))))
   (SEM (LAM X ("CLASS1" X))) (PARAM 1.0))
  ((KEY 85) (PHON WANAL-GU) (MORPH N) (SYN ((BCAT N) (FEATS ((CASE DAT)))))
   (SEM "BOOMERANG") (PARAM 1.0))
  ((KEY 86) (PHON BANUL-DIN-GU) (MORPH N)
   (SYN
    ((((BCAT NP) (FEATS ((POSS GEN) (CASE DAT)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((POSS GEN) (CASE DAT)))))
     (DIR FS) (MODAL ALL) ((BCAT N) (FEATS ((POSS GEN) (CASE DAT))))))
   (SEM (LAM X (LAM Y (("OF" X) Y)))) (PARAM 1.0))
  ((KEY 87) (PHON YARA-NU-N-DIN-GU) (MORPH N)
   (SYN ((BCAT N) (FEATS ((POSS GEN) (CASE DAT))))) (SEM "MAN") (PARAM 1.0))
  ((KEY 88) (PHON "bagul wanal-gu") (MORPH DY)
   (SYN
    (((BCAT NP) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT NP) (FEATS ((CASE DAT))))))
   (SEM (LAM X ("BOOMERANG" X))) (PARAM 1.0))
  ((KEY 89) (PHON M33KA55) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "MUGA")
   (PARAM 1.0))
  ((KEY 90) (PHON M33KO44) (MORPH N) (SYN ((BCAT NP) (FEATS NIL))) (SEM "MUGO")
   (PARAM 1.0))
  ((KEY 91) (PHON NDU21) (MORPH NU)
   (SYN
    ((((BCAT S) (FEATS ((TYP E)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("BEAT" Y) X)))) (PARAM 1.0))
  ((KEY 92) (PHON NDU21) (MORPH NU)
   (SYN
    ((((BCAT S) (FEATS ((TYP A)))) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("BEAT" X) Y)))) (PARAM 1.0))
  ((KEY 93) (PHON NATEX-KAN-KE) (MORPH SH)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((CASE ERG) (AGR PLU)))))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ABS))))))
   (SEM (LAM X (LAM Y (("BITE" X) Y)))) (PARAM 1.0))
  ((KEY 94) (PHON NATEX) (MORPH SH)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((CASE ERG) (AGR SING)))))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS ((CASE ABS))))))
   (SEM (LAM X (LAM Y (("BITE" X) Y)))) (PARAM 1.0))
  ((KEY 95) (PHON MAWA-KAN-KE) (MORPH SH)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
     ((BCAT NP) (FEATS ((CASE ABS) (AGR PLU))))))
   (SEM (LAM X ("DIE" X))) (PARAM 1.0))
  ((KEY 96) (PHON OCHITI-BAON-RA) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE ERG) (AGR PLU) (MOD PRT))))) (SEM "DOGS")
   (PARAM 1.0))
  ((KEY 97) (PHON BAKE) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE ABS) (NUM SING))))) (SEM "CHILD") (PARAM 1.0))
  ((KEY 98) (PHON JONI-BO-RA) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE ABS) (AGR PLU) (MOD PRT))))) (SEM "PERSONS")
   (PARAM 1.0))
  ((KEY 99) (PHON BAKE-BO) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE ABS) (NUM PLU))))) (SEM "CHILDREN")
   (PARAM 1.0))
  ((KEY 100) (PHON OCHITI-NIN-RA) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((CASE ERG) (AGR SING) (MOD PRT))))) (SEM "DOG")
   (PARAM 1.0))
  ((KEY 101) (PHON "Dass sie kommt") (MORPH C)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
      ((BCAT S) (FEATS ((TOP P)))))))
   (SEM (LAM P (P ("COMES" "SHE")))) (PARAM 1.0))
  ((KEY 102) (PHON GLAUBT) (MORPH DE)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR FS) (MODAL ALL) ((BCAT S) (FEATS NIL))))
   (SEM (LAM P (LAM X (("BELIEVE" P) X)))) (PARAM 1.0))
  ((KEY 103) (PHON ER) (MORPH PRO)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (P "HE"))) (PARAM 1.0))
  ((KEY 104) (PHON NICHT) (MORPH F)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL HARMONIC) ((BCAT S) (FEATS NIL))))
   (SEM (LAM P ("NEG" P))) (PARAM 1.0))
  ((KEY 105) (PHON "Die Diplomarbeit") (MORPH N) (SYN ((BCAT NP) (FEATS NIL)))
   (SEM "THE-MA") (PARAM 1.0))
  ((KEY 106) (PHON ZU) (MORPH C)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P P)) (PARAM 1.0))
  ((KEY 107) (PHON SCHREIBEN) (MORPH DE)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))
   (SEM (LAM X (LAM Y (("WRITE" X) Y)))) (PARAM 1.0))
  ((KEY 108) (PHON HAT) (MORPH F)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P P)) (PARAM 1.0))
  ((KEY 109) (PHON "die Studentin") (MORPH N)
   (SYN
    (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))))
   (SEM (LAM P (P "STUDENT"))) (PARAM 1.0))
  ((KEY 110) (PHON GELANWEILT) (MORPH DE)
   (SYN (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT S) (FEATS NIL))))
   (SEM (LAM P ("BORE" P))) (PARAM 1.0))
  ((KEY 111) (PHON JANOS) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |3S|)))))
   (SEM "JOHN") (PARAM 1.0))
  ((KEY 112) (PHON LAT-T-A) (MORPH HU)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS ((AGR |3S|))))))
   (SEM (LAM X (LAM Y (("SEE" X) Y)))) (PARAM 1.0))
  ((KEY 113) (PHON OT) (MORPH N)
   (SYN ((BCAT NP) (FEATS ((AGR |3S|) (TYP PRO))))) (SEM "HIM") (PARAM 1.0))
  ((KEY 114) (PHON LAT-OTT) (MORPH HU)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS ((AGR |1S|))))))
   (SEM (LAM X (LAM Y (("SEE" X) Y)))) (PARAM 1.0))
  ((KEY 115) (PHON LAT-OTT) (MORPH HU)
   (SYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR FS) (MODAL ALL) ((BCAT NP) (FEATS ((AGR |2S|))))))
   (SEM (LAM X (LAM Y (("SEE" X) Y)))) (PARAM 1.0))
  ((KEY 116) (PHON ENGEM) (MORPH N) (SYN ((BCAT NP) (FEATS ((AGR |1S|)))))
   (SEM "ME") (PARAM 1.0))
  ((INDEX #:|_G2_772|) (KEY 119) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT SP) (FEATS NIL)))
   (OUTSYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |3S|)))))
     (DIR BS) (MODAL HARMONIC)
     ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((AGR |3S|)))))
      (DIR FS) (MODAL HARMONIC) ((BCAT SP) (FEATS NIL))))))
  ((INDEX #:|_G2_773|) (KEY 120) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT (UP)) (FEATS ((BCONST T)))))
   (OUTSYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((LEXC P)))))
     (DIR BS) (MODAL ALL)
     (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
       (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((LEXC P)))))
      (DIR FS) (MODAL ALL) ((BCAT (UP)) (FEATS ((BCONST T))))))))
  ((INDEX #:|_G2_774|) (KEY 121) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT NP) (FEATS ((HEAVY M)))))
   (OUTSYN
    (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL ALL) ((BCAT (UP)) (FEATS ((BCONST T)))))
     (DIR BS) (MODAL HARMONIC)
     (((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
       (DIR FS) (MODAL ALL) ((BCAT (UP)) (FEATS ((BCONST T)))))
      (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS ((HEAVY M))))))))
  ((INDEX #:|_G2_777|) (KEY 124) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT S) (FEATS NIL)))
   (OUTSYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |2S|)))))
     (DIR BS) (MODAL HARMONIC)
     ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((AGR |2S|)))))
      (DIR FS) (MODAL HARMONIC) ((BCAT S) (FEATS NIL))))))
  ((INDEX #:|_P2_795|) (KEY 142) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT NP) (FEATS NIL)))
   (OUTSYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
     (DIR BS) (MODAL HARMONIC)
     ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL)))
      (DIR FS) (MODAL HARMONIC) ((BCAT NP) (FEATS NIL))))))
  ((INDEX #:|_G2_779|) (KEY 126) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT S) (FEATS NIL)))
   (OUTSYN
    ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
      ((BCAT NP) (FEATS ((AGR |1S|)))))
     (DIR BS) (MODAL HARMONIC)
     ((((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL)
       ((BCAT NP) (FEATS ((AGR |1S|)))))
      (DIR FS) (MODAL HARMONIC) ((BCAT S) (FEATS NIL))))))
  ((INDEX #:|_P2_797|) (KEY 144) (PARAM 1.0) (INSEM LF)
   (OUTSEM (LAM LF (LAM P (P LF)))) (INSYN ((BCAT NP) (FEATS NIL)))
   (OUTSYN
    (((BCAT S) (FEATS NIL)) (DIR FS) (MODAL ALL)
     (((BCAT S) (FEATS NIL)) (DIR BS) (MODAL ALL) ((BCAT NP) (FEATS NIL))))))))
